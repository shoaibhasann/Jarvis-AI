...
function getMode(prompt){
  const codeKeywords = ["code", "algorithm", "function", "program", "script", "bug", "error"];
  
  return codeKeywords.some((k) => prompt.toLowerCase().includes(k)) ? "code" : "chat";
...
export async function askGemini(prompt) {
  try {
    ...
    const genAI = new GoogleGenerativeAI(
      "AIzaSyBOPhC3hGtVgQm7o5O_NoFJcp35ZLpg_Ck"
    );
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
